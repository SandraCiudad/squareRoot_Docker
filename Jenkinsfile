            ...
            steps {
                script {
                    dir("${env.WORKSPACE}") {
                        ...
                        def cppcheck = 'cppcheck --enable=all --inconclusive --xml --xml-version=2 `find "../squareRoot_docker" -name "*.c*" | grep -v ".cccc" | grep -v ".svn" | grep -v ".settings" | grep -v ".cproject"` 2> reports/project_cppcheck.xml'
